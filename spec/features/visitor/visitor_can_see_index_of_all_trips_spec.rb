...
describe 'visitor can see all trips' do
  describe 'on the trips index page' do
    it 'shows all trips along with all their attributes' do
      ...
      expect(page).to have_content(trip.start_date.in_time_zone("MST").strftime("%m/%d/%Y %l:%M %P"))
      expect(page).to have_content(trip.end_date.in_time_zone("MST").strftime("%m/%d/%Y %l:%M %P"))
      ...
      expect(page).to have_content(trip_2.start_date.in_time_zone("MST").strftime("%m/%d/%Y %l:%M %P"))
      expect(page).to have_content(trip_2.end_date.in_time_zone("MST").strftime("%m/%d/%Y %l:%M %P"))
